...
type RouteType = 'tramontana' | 'kabotaj' | 'fortuna' | 'uzun_kabotaj';
type Intent = 'kervan' | 'kara_bayrak' | 'pusula' | 'duman';
type Tactic = 'pruva' | 'ates' | 'manevra' | 'kacis';
...
type GameState = {
  ...
  season: 'yaz' | 'kis';
  lastWhispers: string[];
  activeRumors: Array<{ text: string; currentPorts: string[] }>;
  player: {
    gold: number;
    currentPortId: string;
    renown: string[];
    cargo: CargoItem[];
    ship: { type: string; cargoCapacity: number; durability: number };
...
type TurnResolution = {
  nextState: GameState;
  log: Array<{ label: string; detail: string }>;
  whispers: string[];
  combat?: {
    result: string;
    tactic: string;
...
export default function Page() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/bootstrap`)
      ...
      .catch(() => setError('Mock server bootstrap verisi alınamadı. Önce serverı çalıştır.'));
